...
export default function App() {
  // Application Roles Switching (Mock Quick-Bar for previewing)
  ...
  // Switch role directly from Login or Top Dev Panel
  const handleSwitchRole = (role: 'parent' | 'teacher') => {
    setActiveRole(role);
  };
  ...
  // TEACHER CALLBACKS
  const handleApproveStudent = (id: string, assignedRollId: string) => {
    setStudents(prev => prev.map(s => {
      if (s.id === id) {
        return {
          ...s,
          status: 'approved',
          studentId: assignedRollId
        };
      }
      return s;
    }));
  ...
  const handleUpdateStudentGrades = (id: string, updatedGrades: SubjectGrade[]) => {
    setStudents(prev => prev.map(s => {
      if (s.id === id) {
        return {
          ...s,
          academicGrades: updatedGrades
        };
      }
      return s;
    }));
  ...
  const handleUpdateStudentAttendance = (id: string, presentDays: number, totalDays: number) => {
    setStudents(prev => prev.map(s => {
      if (s.id === id) {
        const perc = parseFloat(((presentDays / totalDays) * 100).toFixed(1));
        return {
          ...s,
          attendance: {
            present: presentDays,
  ...
  const handleAddNotice = (title: string, content: string, category: 'Urgent' | 'General' | 'Event') => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-start text-sans select-none antialiased md:py-6">
      ...
      {/* 🚀 QUICKROLE SWITCHER & DIAGNOSTIC TOOLBAR FOR CONVENIENCE */}
      <div className="w-full max-w-lg bg-slate-950 text-white rounded-t-2xl py-3 px-4 flex justify-between items-center text-xs font-bold border-b border-indigo-900 shadow-md">
        <div className="flex items-center gap-1.5 text-indigo-300">
          ...
          <span>Interactive Preview Console:</span>
        </div>
        <div className="flex gap-1">
          <button
            onClick={() => handleSwitchRole('parent')}
            className={`px-3 py-1.5 rounded-lg text-[10px] font-extrabold uppercase transition-all tracking-wider shrink-0 cursor-pointer ${
              activeRole === 'parent' 
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[10px] font-extrabold uppercase transition-all tracking-wider shrink-0 cursor-pointer ${
              activeRole === 'teacher' 
                ? 'bg-[#326852] text-white shadow-xs' 
                : 'text-slate-400 bg-slate-900 hover:text-white'
            }`}
          >
        ...
        <header className="fixed top-0 max-w-lg w-full z-40 bg-[#091426] text-white flex items-center justify-between px-4 h-16 shadow-md">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-sans font-extrabold text-sm md:text-base tracking-tight text-white">
              {activeRole === 'parent' ? 'MyHLES Parent Portal' : "St. Xavier's Administrator"}
            </h1>
          </div>
        ...
        <div className="flex-grow flex flex-col pt-16 bg-[#f8f9ff]">
          ...
          {activeRole === 'teacher' ? (
            ...
            <TeacherPortal
              ...
              onUpdateGrades={handleUpdateStudentGrades}
            ...
            <ParentLogin 
              ...
              onSwitchToTeacher={() => handleSwitchRole('teacher')}
            />
          ) : !isOnboardingCompleted ? (
